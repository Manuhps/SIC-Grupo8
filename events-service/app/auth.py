...
def verify_token(authorization: str | None = Header(default=None)) -> Dict[str, Any]:
    """Verifica se o Token JWT é válido e retorna os dados do utilizador."""
...
def require_organizador(user: Dict[str, Any]) -> Dict[str, Any]:
    """Verifica se o utilizador é Organizador ou Admin."""
...
def require_estudante(user: Dict[str, Any]) -> Dict[str, Any]:
    """Verifica se o utilizador é Estudante ou Admin."""
    user_type = user.get("tipo")
    if user_type not in ("estudante", "admin"):
        _forbidden(
            "Acesso negado. Apenas estudantes ou administradores podem aceder a este recurso."
        )
    return user
